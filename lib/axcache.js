...
import { CacheWithTTL } from './cache/CacheWithTTL';
import { generateCacheKey, createNormalizedConfig } from './utils/cacheUtils';
...
const createAxcache = (options = {}) => {
  ...
  instance.interceptors.response.use(
    (response) => {
      if (response.config.method?.toLowerCase() === 'get') {
        ...
        // Create a normalized version of the response object for caching
        // by removing circular references and non-serializable properties
  ...
  instance.getCacheStats = () => cache.getStats();
